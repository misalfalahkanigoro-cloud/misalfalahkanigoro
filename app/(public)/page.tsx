...
import type { NewsPost, HeadmasterGreeting, Publication, Gallery, Achievement, Download } from '@/lib/types';
...
const Home: React.FC = () => {
    ...
    const [hasActivePpdbWave, setHasActivePpdbWave] = useState(false);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [
                ...
                api.getPublications({ page: 1, pageSize: 3 }),
                api.getGalleries({ page: 1, pageSize: 3 }),
                api.getAchievements({ page: 1, pageSize: 3 }),
                api.getDownloads(),
                api.getHeadmasterGreeting(),
                fetch('/api/ppdb/active-wave'),
            ]);

            if (newsResult.status === 'fulfilled') {
                setNews(extractItems<NewsPost>(newsResult.value));
            } else {
                console.error('Homepage news fetch failed:', newsResult.reason);
            }

            if (publicationResult.status === 'fulfilled') {
                setPublications(extractItems<Publication>(publicationResult.value));
            } else {
                console.error('Homepage publications fetch failed:', publicationResult.reason);
            }

            if (galleryResult.status === 'fulfilled') {
                setGalleries(extractItems<Gallery>(galleryResult.value));
            } else {
                console.error('Homepage galleries fetch failed:', galleryResult.reason);
            }

            if (achievementResult.status === 'fulfilled') {
                setAchievements(extractItems<Achievement>(achievementResult.value));
            } else {
                console.error('Homepage achievements fetch failed:', achievementResult.reason);
            }

            if (downloadResult.status === 'fulfilled') {
                setDownloads(extractItems<Download>(downloadResult.value).slice(0, 3));
            } else {
                console.error('Homepage downloads fetch failed:', downloadResult.reason);
            }

            if (greetingResult.status === 'fulfilled') {
                setGreeting((greetingResult.value as HeadmasterGreeting) || null);
            } else {
                console.error('Homepage greeting fetch failed:', greetingResult.reason);
            }

            if (activeWaveResult.status === 'fulfilled' && activeWaveResult.value.ok) {
                try {
                    const activeWaveData = await activeWaveResult.value.json();
                    setHasActivePpdbWave(Boolean(activeWaveData?.active));
                } catch (error) {
                    console.error('Homepage active wave parse failed:', error);
                    setHasActivePpdbWave(false);
                }
            } else {
                if (activeWaveResult.status === 'rejected') {
                    console.error('Homepage active wave fetch failed:', activeWaveResult.reason);
                }
                setHasActivePpdbWave(false);
            }
    ...
    const newsCards: ContentCardItem[] = news.map((item) => ({
        id: item.id,
        title: item.title,
    ...
    const publicationCards: ContentCardItem[] = publications.map((item) => ({
        id: item.id,
        title: item.title,
        href: `/publikasi/${item.slug}`,
        coverUrl: item.coverUrl || null,
        date: item.publishedAt || item.createdAt,
        excerpt: item.excerpt || item.description || item.content?.substring(0, 120),
        badge: 'Publikasi',
    }));

    const galleryCards: ContentCardItem[] = galleries.map((item) => ({
        id: item.id,
        title: item.title,
    ...
    const achievementCards: ContentCardItem[] = achievements.map((item) => ({
        id: item.id,
        title: item.title,
        href: `/prestasi/${item.slug}`,
        coverUrl: item.coverUrl || null,
        date: item.achievedAt || item.createdAt,
        excerpt: item.description || item.eventName || 'Prestasi terbaru madrasah.',
        badge: 'Prestasi',
    }));

    const downloadCards: ContentCardItem[] = downloads.map((item) => ({
        id: item.id,
        title: item.title,
        href: '/download',
        coverUrl: item.coverUrl || null,
