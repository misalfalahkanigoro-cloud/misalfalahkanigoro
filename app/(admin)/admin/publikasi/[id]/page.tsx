...
import { ArrowLeft, Save, Trash2, FileText, Megaphone, Newspaper, Paperclip, ImagePlus } from 'lucide-react';
...
const AdminPublicationEditPage: React.FC = () => {
    ...
    useEffect(() => {
        if (!isNew && id) {
            const fetchData = async () => {
                try {
                    const res = await adminApi.getPublicationDetail(id);
                    if (res) {
                        const coverFromMedia = (res as Publication).media?.find((item) => item.isMain)?.mediaUrl || '';
                        setForm({
                            ...res,
                            coverUrl: (res as Publication).coverUrl || coverFromMedia,
    ...
    const handleSave = async () => {
        ...
        try {
            const payload: Partial<Publication> = {
                ...
                slug: form.slug?.trim(),
                type: (form.type || 'article') as PublicationType,
            ...
            if (isNew) {
                await adminApi.createPublication(payload as any);
            } else {
                await adminApi.updatePublication(id, payload as any);
            }
    ...
    const handleFileUpload = (url: string, info?: any) => {
        const resourceType = String(info?.resource_type || '').toLowerCase();
        if (resourceType && resourceType !== 'image' && resourceType !== 'video') {
            alert('Media publikasi hanya mendukung gambar atau video.');
            return;
        }

        const mediaType: MediaItem['mediaType'] = resourceType === 'video' ? 'video' : 'image';
    ...
    const handleCoverUpload = (url: string, info?: any) => {
        const resourceType = String(info?.resource_type || '').toLowerCase();
        if (resourceType && resourceType !== 'image') {
            alert('Cover harus berupa gambar.');
            return;
        }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="rounded-3xl border border-emerald-900/20 bg-white p-8 shadow-sm dark:border-white/10 dark:bg-white/5">
                            ...
                            {(!form.media || form.media.length === 0) ? (
                                <div className="text-center py-8 bg-gray-50 dark:bg-white/5 rounded-xl border border-dashed border-gray-200 dark:border-white/10">
                                    <p className="text-gray-400 text-sm">Belum ada media yang diupload</p>
                                </div>
                            ) : (
                                    ...
                                    {form.media.map((item, idx) => (
                                        <div key={idx} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/5 group">
                                            ...
                                            {item.mediaType === 'image' && (
                                                <button
                                                    type="button"
                                                    onClick={() => setForm((prev) => ({ ...prev, coverUrl: item.mediaUrl }))}
                                                    className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition ${
                                                        form.coverUrl === item.mediaUrl
                                                            ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300'
                                                            : 'bg-white text-emerald-700 border border-emerald-200 hover:bg-emerald-50 dark:bg-white/5 dark:border-white/10 dark:text-emerald-300'
                                                    }`}
                                                >
                                                    {form.coverUrl === item.mediaUrl ? 'Cover Utama' : 'Jadikan Cover'}
                                                </button>
                                            )}
                                            <button
                    ...
                    <div className="space-y-6">
                        <div className="rounded-3xl border border-emerald-900/20 bg-white p-8 shadow-sm dark:border-white/10 dark:bg-white/5 space-y-6">
                            ...
                            <div className="space-y-3">
                                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300">Cover Publikasi</label>
                                <div className="rounded-2xl border border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-3">
                                    {form.coverUrl ? (
                                        ...
                                        <div className="w-full h-40 rounded-xl border border-dashed border-gray-300 dark:border-white/10 flex items-center justify-center text-gray-400">
                                ...
                                <MediaUploadButton
                                    folder="mis-al-falah/publications/cover"
                                    label="Upload Cover"
                                ...
                                <input
                                    ...
                                    className="w-full px-3 py-2 rounded-xl bg-gray-50 dark:bg-white/5 border border-transparent focus:border-emerald-500 focus:ring-0 transition text-xs"
                                />
                                {form.coverUrl && (
                                    <button
                                        type="button"
                                        onClick={() => setForm({ ...form, coverUrl: '' })}
                                        className="w-full py-2 rounded-xl border border-red-200 text-red-600 text-xs font-bold hover:bg-red-50 transition"
                                    >
                                        Hapus Cover
                                    </button>
                                )}
                                <p className="text-[11px] text-gray-500 dark:text-gray-400">
                                    Cover disimpan ke tabel <span className="font-mono">media_items</span> sebagai <span className="font-mono">is_main = true</span>.
                                </p>
                            </div>
