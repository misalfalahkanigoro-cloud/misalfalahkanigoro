...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (listBuckets) {
            const { data, error } = await dbAdmin().storage.listBuckets();
            if (error) throw error;
        ...
        return NextResponse.json({ error: 'Gagal memuat storage' }, { status: 500 });
    }
}
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ error: 'Gagal menghapus file storage' }, { status: 500 });
    }
}
