...
export async function GET() {
    try {
        ...
        const { data: news, error: newsError } = await dbAdmin()
            .from('news_posts')
        ...
        const { data: pubs, error: pubsError } = await dbAdmin()
            .from('publications')
        ...
        const { data: achievements, error: achError } = await dbAdmin()
            .from('achievements')
        ...
        // Map and merge
        const pinnedNews: PinnedItem[] = (news || []).map((p: any) => ({
            id: p.id,
            title: p.title,
            slug: p.slug,
        ...
        const pinnedPubs: PinnedItem[] = (pubs || []).map((p: any) => ({
            id: p.id,
            title: p.title,
            slug: p.slug,
            coverUrl: null,
        ...
        const pinnedAchievements: PinnedItem[] = (achievements || []).map((a: any) => ({
            id: a.id,
            title: a.title,
            slug: a.slug,
            coverUrl: null,
