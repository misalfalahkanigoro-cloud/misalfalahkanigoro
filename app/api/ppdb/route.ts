...
export async function POST(request: NextRequest) {
    try {
        ...
        const { data: activeWave, error: waveError } = await dbAdmin()
            .from('ppdb_waves')
            .select('id')
            .eq('is_active', true)
        ...
        if (files.length && registration?.id) {
            const normalized = files
                .map((file: any) => ({
                    ...
                    storage_provider: file.storageProvider || file.provider || null,
                    storage_bucket: file.storageBucket || file.bucket || null,
                    storage_path: file.storagePath || file.path || file.publicId || null,
                    file_url: file.fileUrl,
                }))
                .filter((file: any) => file.file_type && file.file_url);

            if (normalized.length) {
                const { error: fileError } = await dbAdmin()
                    .from('ppdb_files')
                    .insert(normalized);
